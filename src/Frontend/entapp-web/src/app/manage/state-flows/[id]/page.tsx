...
import { getFlowDefinition } from "@/lib/api/state-flow";
...
export default function ManageFlowDesignerPage() {
  ...
  if (error) {
    return (
      <div style={{ padding: 48, textAlign: "center", color: "#ef4444" }}>
        <h2>Hata</h2>
        <p>{error}</p>
        <button onClick={() => router.push("/manage/state-flows")} style={{ marginTop: 16, color: "#8b5cf6", background: "none", border: "none", cursor: "pointer" }}>
          ← Listeye Dön
        </button>
      </div>
    );
    ...
    <div className="designer-page">
      <div className="designer-page__header">
        <button
          ...
          onClick={() => router.push("/manage/state-flows")}
        >
          ← Geri
        </button>
